...
parfor template_count = 1:length(template_list)
    ...
    for station_count = 1:length(station_list)
        ...
        fprintf('Producing Grand Correlation for %s %s %s, %s to %s\n',...
            single_station.template,NET,STA,datestr(first_date),...
            datestr(last_date));
        
        [BHZ,BHN,BHE] = produceAlignedCorrelations...
            (NET,single_station.channel_list{1},...
            single_station.channel_list{2},...
            single_station.channel_list{3},...
            STA,...
            first_date,...
            last_date,...
            single_station.pWaveArrival,...
            single_station.sWaveArrival,...
            window,...
            single_station.template);
        tri = [BHZ,BHN,BHE];
        correlation_savename =...
            sprintf('./CO/%s/%s_%s_correlation.%s.to.%s.mat',...
            single_station.template,single_station.network,...
            single_station.station,datestr(first_date),datestr(last_date));
...
for template_count = 1:length(template_list)
    station_list = template_list{template_count};
    template_BHE_savename = ...
        sprintf('./CO/%s/BHE_Combined_Correlation.%s.to.%s.mat',...
        station_list(1).template,datestr(first_date),datestr(last_date));
    template_BHN_savename = ...
        sprintf('./CO/%s/BHN_Combined_Correlation.%s.to.%s.mat',...
        station_list(1).template,datestr(first_date),datestr(last_date));
    template_BHZ_savename = ...
        sprintf('./CO/%s/BHZ_Combined_Correlation.%s.to.%s.mat',...
        station_list(1).template,datestr(first_date),datestr(last_date));
    for station_count = 1:length(station_list)
        template_channels = station_list(station_count).channel_list;
        station_list(station_count).template_channels = template_channels;
        ...
        if station_count == 1
            ...
            if ~isempty(waveform(data(1)))
                BHE = cat(BHE,data(1));
            end
            if ~isempty(waveform(data(2)))
                BHN = cat(BHN,data(2));
            end
            if ~isempty(waveform(data(3)))
                BHZ = cat(BHZ,data(3));
            end
        end
    end
