...
for template_count = 1:length(template_list)
    ...
    for station_count = 1:length(single_template);
        ...
        location = '*';%station_specific_template.location;
        for chan_count = 1:numberofchannels
            ...
            directory_check = sprintf('%s/%s/%s/Templates',network,station,channel);
            if exist(directory_check,'dir') ~= 7;
                mkdir(directory_check);
                fprintf('Template directory created \n');
            end
            directory_check = sprintf('%s/%s/%s/Templates/%s',network,station,channel,template);
            if exist(directory_check,'dir') ~= 7;
                mkdir(directory_check);
                fprintf('Template directory created \n');
            end
            ...
            template_savename = sprintf('%s/%s/%s/Templates/%s/template_%s.%s.%s.mat',network,station,channel,template,template,station,channel);
            if exist(template_savename,'file') == 2
                ...
                while isempty(wf_Temp) == 1
                    try
                        ...
                        wf_Temp = irisFetch.Traces(network,station,location, channel, datestr(starttime), datestr(endtime),'http://service.ncedc.org','verbose');
                ...
                data_savename = sprintf('%s/%s/%s/Templates/%s/data_%s.%s.%s.mat',network,station,channel,template,template,station,channel);
		...
		upper_band =get(wf_Temp,'freq')*(0.8/2);
