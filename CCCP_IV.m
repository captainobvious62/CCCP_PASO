...
all_arrivals_savename = sprintf(...
    './Results/Arrivals/Combined_Arrivals_%s_to_%s.mat',...
    datestr(first_date),datestr(last_date));
all_arrivals = [];
for template_count = 1:length(template_list)
    ...
    for station_count = 1:length(station_list)
        ...
        arrival_savename = sprintf...
            ('./%s/%s/arrivals/%s/arrival_times.%s.to.%s.mat',...
            single_station.network,single_station.station,...
            single_station.template,...
            datestr(first_date),datestr(last_date));
        arrival_output_savename = sprintf...
            ('./Results/Arrivals/%s_%s_%s_arrival_times.%s.to.%s.txt',...
            single_station.network,single_station.station,...
            single_station.template,...
            datestr(first_date),datestr(last_date));
        ...
        if isempty(data) ~= 1;
            ...
            header = {sprintf('P Arrivals, %s %s\n',...
                ...
                sprintf('S Arrivals, %s %s\n',...
                single_station.template,STA),'S - P'};
            arrivals = {datestr(data(:,1)),datestr(data(:,2)),...
                num2str(data(:,3))};
            arrivals = {cellstr(arrivals{1}),...
                cellstr(arrivals{2}),cellstr(arrivals{3})};
