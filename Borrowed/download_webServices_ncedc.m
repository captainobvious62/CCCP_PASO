%
%
function download_webServices_ncedc(NET,STA,LOC,YEAR,DDD,CHA_E,CHA_N,CHA_Z)
...
%  DDD = datevec2doy([YEAR,MONTH,DDD,0,0,0])
...
MONTH=ymd(2);
MDAY=ymd(3);
START=[num2str(YEAR),'-',num2str(MONTH,'%02d'),'-',num2str(MDAY,'%02d'),'T00:00:00.000000'];
END=[num2str(YEAR),'-',num2str(MONTH,'%02d'),'-',num2str(MDAY,'%02d'),'T23:59:59.999999'];
['http://service.ncedc.org/fdsnws/dataselect/1/query?net=',NET,'&sta=',STA,'&loc=',LOC,'&cha=',CHA_E,'&start=',START,'&end=',END];
...
if exist([NET,'/',STA,'/mseed/',STA,'.',CHA_E,'.',num2str(YEAR),'.',num2str(DDD,'%03d')],'file') == 0 && check_uptime(STA,NET,CHA_E,LOC,doy2date(DDD,YEAR)) == 1
    fprintf([NET,'/',STA,'/mseed/',STA,'.',CHA_E,'.',num2str(YEAR),'.',num2str(DDD,'%03d'),'\n'])
    try
        %%%Try to download data.
        urlwrite(['http://service.ncedc.org/fdsnws/dataselect/1/query?net=',NET,...
